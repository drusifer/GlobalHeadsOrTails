...
def test_production_auth(tmp_path):
    ...
    # Create a temporary copy of the production keys file to avoid modifying it.
    # This ensures test isolation.
    prod_keys_path = Path.cwd() / "tag_keys.csv"
    temp_keys_path = tmp_path / "temp_tag_keys.csv"
    if prod_keys_path.exists():
        shutil.copy(prod_keys_path, temp_keys_path)
    else:
        # Create an empty file with header if it doesn't exist, so the test can run.
        temp_keys_path.write_text("uid,picc_master_key,app_read_key,sdm_mac_key,status,provisioned_date,notes\n")
    
    try:
        seq = create_sequence_logger("ProductionAuth")
        with CardManager(sequence_logger=seq, reader_index=0) as card:
            # Select PICC
            ...
            key_mgr = CsvKeyManager(csv_path=str(temp_keys_path))
            ...
            if (sw1, sw2) == (0x91, 0x00):
                ...
                return False
                pytest.fail(f"ChangeKey failed with SW={sw1:02X}{sw2:02X}")
