...
1. **Authentication Delay (SW=91AD)**
   - Tag enforces delay after failed authentication attempts
   - Delay may persist in same session
   - **Fresh tap likely resets delay counter** ⭐
   - May need 1-2 second wait or reconnection

2. **Phase 1 State Expiration**
   - Expires after ~0.5 seconds
   - Must send Phase 2 immediately

3. **Command 0x51 Recognition**
   - Returns SW=91CA (Command Aborted) or SW=91AE
   - Command exists but wrong state/parameters

4. **Command 0x77 Recognition**
   - Returns SW=917E (Length Error)
   - Command exists and recognized, needs correct data format
