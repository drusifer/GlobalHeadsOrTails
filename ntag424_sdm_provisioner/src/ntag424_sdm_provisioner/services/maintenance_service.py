...
class TagMaintenanceService:
    ...
    def factory_reset(self) -> bool:
        ...
        try:
            ...
            auth_key, key_source = self._get_best_key(uid_bytes)
            ...
            try:
                with AuthenticateEV2(auth_key, 0)(self.card) as auth_conn:
                    ...
                    if auth_key == FACTORY_KEY:
                        ...
                        self._update_key_manager(  # type: ignore[call-arg]
            uid_bytes, "factory")
                        return True
    ...
    def _get_best_key(self, uid_bytes: bytes) -> tuple[bytes, str]:
    ...
    def _update_key_manager(self, uid_bytes: bytes):
        """Update KeyManager with new tag status."""
        try:
            # Try to delete existing entry
            for key_no in [0, 1, 2, 3, 4]:
                self.key_mgr.delete_key(uid_bytes, key_no)  # type: ignore[attr-defined]
