...
class GetChipVersion(ApduCommand):
    ...
    GET_VERSION_APDU: ClassVar[list[int]] = [0x90, 0x60, 0x00, 0x00, 0x00]
    ...
    def sequence_description(self) -> str:
        return "Read chip version info including UID"
    ...
    def parse_response(self, data: bytes, _sw1: int, _sw2: int) -> Ntag424VersionInfo:
        ...
        if len(data) not in [28, 29]:
            raise ValueError(
                f"Received incomplete version data. Expected 28 or 29 bytes, got {len(data)}"
            )
