...
class ChangeKey(AuthApduCommand):
    ...
    def __init__(
        self, key_no_to_change: int, new_key: bytes, old_key: bytes, key_version: int = 0x00
    ):
    ...
    def _build_key_data(self) -> bytes:
        ...
        if self.key_no == 0:
            ...
            xored = bytes(a ^ b for a, b in zip(self.new_key, self.old_key, strict=True))
    ...
    def execute(self, auth_conn: AuthenticatedConnection) -> SuccessResponse:
        ...
        return auth_conn.send(self)  # type: ignore[no-any-return]
