...
class MockCardConnection:
    ...
    def _handle_iso_select_file(self, p1: int, p2: int, data: bytes) -> tuple[list[int], int, int]:
        ...
        # First, check if selecting PICC application by DF name
        ...
        # Select EF by file ID (within already-selected application)
    ...
    def _handle_proprietary_command(self, apdu: list[int]) -> tuple[list[int], int, int]:
        ...
        # GetChipVersion (90 60)
        if ins == 0x60:
            return self._handle_get_chip_version()
        
        # GetAdditionalFrame (90 AF) - can be GetChipVersion continuation or Auth Phase 2
        if ins == 0xAF:
            # Check if GetChipVersion is in progress
            if self.get_version_part > 0:
                return self._handle_get_chip_version()
            # Otherwise, it's AuthenticateEV2Part2
            if self._rndB_prime is not None:
    ...
    def _handle_get_chip_version(self) -> tuple[list[int], int, int]:
        ...
        if self.get_version_part == 0:
            ...
            self.get_version_part = 1
            hw_data = [
                0x04,  # hw_vendor_id (NXP)
                0x04,  # hw_type (NTAG424)
                0x00,  # hw_subtype
                0x30,  # hw_major_version (48 = 0x30)
                0x00,  # hw_minor_version
                0x90,  # hw_storage_size (416 bytes = 0x1A0, truncated to 0x90)
                0x05,  # hw_protocol
            ]
            return hw_data, 0x91, 0xAF  # Additional frame
        elif self.get_version_part == 1:
            # Part 2: Software info (7 bytes)
            self.get_version_part = 2
            sw_data = [
                0x04,  # sw_vendor_id (NXP)
                0x04,  # sw_type (NTAG424)
                0x00,  # sw_subtype
                0x01,  # sw_major_version
                0x02,  # sw_minor_version
                0x11,  # sw_storage_size (17 = 0x11)
                0x05,  # sw_protocol
            ]
            return sw_data, 0x91, 0xAF  # Additional frame
        elif self.get_version_part == 2:
            # Part 3: Production info (14 bytes for mock)
            self.get_version_part = 0  # Reset for next time
            prod_data = [
                0x04, 0x3F, 0x68, 0x4A, 0x2F, 0x70, 0x80,  # UID (7 bytes)
                0xCF, 0x39, 0xD4, 0x49,  # Batch number (4 bytes)
                0x00,  # fab_key
                0x34,  # cw_prod (calendar week 52 = 0x34)
                0x20,  # year_prod (32 = 0x20)
            ]
