...
class TraceBasedSimulator:
    ...
    def _build_trace_database(self) -> dict[str, tuple[list[int], int, int]]:
        ...
        traces["GET_ADDITIONAL_FRAME_2"] = (
            [0x04, 0x51, 0x66, 0x4A, 0x2F, 0x70, 0x80, 0xCF, 0x39, 0xD4, 0x49, 0x80, 0x34, 0x20],
            0x91, 0x00
        )
        ...
        # ===== PART 6: Setup URL (NDEF + SDM) - After Provisioning =====

        # Auth Phase 1 (NEW Key 0 for Setup URL)
        # From SUCCESSFUL NDEF TUI.log.txt line 53
        traces["AUTH_PHASE1_5"] = (
            bytes.fromhex("AE125987350A6A35A4FC3434ADC52796"),
            0x91, 0xAF
        )

        # Auth Phase 2 (NEW Key 0)
        # From SUCCESSFUL NDEF TUI.log.txt line 65
        traces["AUTH_PHASE2_5"] = (
            bytes.fromhex("85E8D8FD6557667FFCCA80ADB6DADE42C2E98B35536AFEA71ADA8C39AB63802F"),
            0x91, 0x00
        )

        # ChangeFileSettings (SDM configuration)
        # From SUCCESSFUL NDEF TUI.log.txt line 114
        traces["90_5F_1"] = (
            bytes.fromhex("6393BAF61A2726F8"),  # 8-byte CMAC response
            0x91, 0x00
        )

        # ===== PART 7: NDEF Operations =====
...
class MockNTag424CardConnection:
    ...
    def send(self, command):
        ...
        if isinstance(command, AuthApduCommand):
            return command.parse_response(bytes(full_data))
        else:
