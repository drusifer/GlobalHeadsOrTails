...
def temp_csv_files():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        yield str(csv_path), str(backup_path), str(backup_dir)


@pytest.fixture
def key_manager(temp_csv_files):
    """Create a CsvKeyManager instance with temporary files."""
    csv_path, backup_path, backup_dir = temp_csv_files
    return CsvKeyManager(csv_path=csv_path, backup_path=backup_path, timestamped_backup_dir=backup_dir)
...
class TestCsvKeyManager:
    ...
    def test_initialization_creates_files(self, temp_csv_files):
        ...
        # Create key manager
        key_mgr = CsvKeyManager(csv_path=csv_path, backup_path=backup_path, timestamped_backup_dir=backup_dir)
    ...
    def test_timestamped_backup_created_on_save(self, key_manager, test_uid, temp_csv_files):
        """Saving keys should create a timestamped backup file."""
        csv_path, backup_path, backup_dir = temp_csv_files
        backup_dir_path = Path(backup_dir)

        # Initially no backups
        backup_files = list(backup_dir_path.glob("tag_keys_*.csv"))
        assert len(backup_files) == 0

        # Save keys
        keys = key_manager.generate_random_keys(test_uid)
        key_manager.save_tag_keys(test_uid, keys)

        # Should have created one timestamped backup
        backup_files = list(backup_dir_path.glob("tag_keys_*.csv"))
        assert len(backup_files) == 1

        # Backup filename should match pattern: tag_keys_YYYYMMDD_HHMMSS.csv
        backup_file = backup_files[0]
        assert backup_file.name.startswith("tag_keys_")
        assert backup_file.name.endswith(".csv")

        # Backup should contain the saved keys
        with backup_file.open('r') as f:
            content = f.read()
        assert test_uid.hex().upper() in content
        assert keys.picc_master_key in content

    def test_timestamped_backup_on_each_state_change(self, key_manager, test_uid, temp_csv_files):
        """Each state change should create a separate timestamped backup."""
        csv_path, backup_path, backup_dir = temp_csv_files
        backup_dir_path = Path(backup_dir)
        ...
        # Third save (another state change)
        time.sleep(1.1)  # Ensure different timestamp
        keys3 = key_manager.generate_random_keys(test_uid)
        keys3.status = "re-provisioned"
        key_manager.save_tag_keys(test_uid, keys3)

        backup_files = sorted(backup_dir_path.glob("tag_keys_*.csv"))
        assert len(backup_files) == 3

        # Verify backups have different timestamps
        timestamps = [f.name for f in backup_files]
        assert len(set(timestamps)) == 3  # All unique

    def test_timestamped_backup_multiple_tags(self, key_manager, temp_csv_files):
        """Timestamped backups should contain all tags in database."""
        csv_path, backup_path, backup_dir = temp_csv_files
        backup_dir_path = Path(backup_dir)
        ...
        # Should have 2 backups (one per save)
        backup_files = sorted(backup_dir_path.glob("tag_keys_*.csv"))
        assert len(backup_files) == 2

        # Latest backup should contain both tags
        latest_backup = backup_files[-1]
        with latest_backup.open('r') as f:
            content = f.read()

        assert uid1.hex().upper() in content
        assert uid2.hex().upper() in content
...
class TestProvisionTagContextManager:
    ...
    def test_provision_success(self, key_manager, test_uid):
        ...
        # Use context manager with URL - simulates full provisioning
        test_url = "https://example.com/tap?uid={UID}&ctr={CTR}&mac={MAC}"
        with key_manager.provision_tag(test_uid, url=test_url) as keys:
            ...
            # (In real code: ChangeKey commands + SDM configuration here)
            pass
        ...
        assert final_keys.notes == test_url
    ...
    def test_provision_atomic_commit(self, key_manager, test_uid):
        ...
        class ProvisioningSimulator:
            ...
            def __init__(self, key_mgr, uid, url):
            ...
            def provision(self):
                with self.key_mgr.provision_tag(self.uid, url=self.url) as keys:
                    # Check state during provisioning
                    db_keys = self.key_mgr.get_tag_keys(self.uid)
                    assert db_keys.status == "pending"
                    self.checked_pending = True

        simulator = ProvisioningSimulator(key_manager, test_uid, test_url)
        simulator.provision()
